...
setwd("/Volumes/xiaowang/zhoushan/新分析/diversity/16S18SV91_interaction/NCM/prokaryote")

phyto_asv <- read.table("sequences_rarefied_20240222.txt", header = TRUE, sep = "\t" )

phyto_asv1 <- phyto_asv %>%
  select(9:58)
...
grid.text(x=unit(0,'npc')-unit(3,'lines'),label='Frequency of Occurence',gp=gpar(fontface=2),rot=90) 
...
N <- mean(apply(spp, 1, sum))
m_value <- coef(m.fit)
results <- data.frame(N = N, m = m_value)

# 保存到 CSV 文件
write.csv(results, file = "N_and_m.csv", row.names = FALSE)
